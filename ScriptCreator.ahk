...
clickCapture() {
    global name, xCoord, yCoord, HK, Description, Speak, Clicks
    gui, capture:add, text,, Ingrese el nombre del script
    gui, capture:add, edit, vName
    ...
    sleep 250
    gui, capture:show,, Configuración de la coordenada
}
...
Data() {
    ...
    if (!HK) {
        ...
        if (Clicks == "Click izquierdo") {
            action = click
        } else if (Clicks == "Doble click") {
            action = click, 2
        } else if (Clicks == "Click derecho") {
            action = click, right
        } else {
            action =
        }
...
scriptCompile() {
    ...
    Sleep 250
    runWait cmd.exe /c AHK\Compiler\Ahk2Exe.exe /in "Coordenadas.ahk" /out "%name%.exe",, Hide
    sleep 1000
